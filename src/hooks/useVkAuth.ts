...
export function UserAvatar({ size = 'md', showName = false }: UserAvatarProps) {
  ...
  return (
    <>
      {/* Аватар-кнопка — открывает карточку профиля */}
      <button
        type="button"
      ...
      {/* 🆕 Модалка профиля через ПОРТАЛ в document.body:
          backdrop-blur / sticky / transform у предков больше не ломают позиционирование */}
      {showProfile &&
        createPortal(
          <div
