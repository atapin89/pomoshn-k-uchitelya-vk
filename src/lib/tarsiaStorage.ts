...
export function parseTarsiaPuzzleFile(text: string): TarsiaPuzzle | null {
  try {
    ...
    const puzzleData = data.format === 'pomoshnik-uchitelya-tarsia' ? data.puzzle : data;
    
    if (!puzzleData || typeof puzzleData.title !== 'string' || !Array.isArray(puzzleData.pairs)) {
      return null;
    }
    ...
    return {
      ...
      shape: ['triangle', 'hexagon', 'domino'].includes(puzzleData.shape)
        ? puzzleData.shape
        : 'triangle',
      pairs: validPairs,
